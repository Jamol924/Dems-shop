...
function Home({ category, dataCard }) {
  ...
  const [glavne, setGlavne] = useState(dataCard.subtype);
  const handleGlavneChange = (el) => {
    setGlavne(el.target.value);
  };
  const [arend, setArend] = useState(dataCard.sell_property);
  ...
  const [tips, setTips] = useState(dataCard.home_plan);
  const handleTips = (el) => {
  ...
  const [novy, setNovy] = useState(dataCard.situation);
  ...
  const [has, setHas] = useState(dataCard.has_furniture);
  const handleHas = (event) => {
    setHas(event.target.value);
  };
  const [otremont, setOtremont] = useState(dataCard.house_renovation);
  const handleRem = (event) => {
    setOtremont(event.target.value);
  };
  const [kir, setKir] = useState(dataCard.construction);
  const handleKir = (event) => {
    setKir(event.target.value);
  };
  const [cost, setCost] = useState(dataCard.has_additional_cost);
  ...
  const [ryadom, setRyadom] = useState(dataCard.located_near);
  ...
  const [gorod, setGorod] = useState(dataCard.city_id);
  const [gorods, setGorods] = useState([]);
  const [region, setRegion] = useState(dataCard.region_id);
  ...
  const [personName, setPersonName] = useState(dataCard.appliances);
  ...
  const {
    ...
    defaultValues: {
      name: dataCard.title,
      kol: dataCard.count_room,
      god: dataCard.year_house_build,
      potol: dataCard.celling_height,
      etaj: dataCard.floor,
      etajd: dataCard.floor_of_house,
      rya: dataCard.located_near,
      raz: dataCard.size,
      dos: dataCard.available_size,
      sen: dataCard.price,
